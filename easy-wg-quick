...
PostUp = nft add chain inet filter postrouting "{ type nat hook postrouting priority 100 ; }"
...
PostUp = nft add rule inet filter postrouting ip protocol tcp tcp flags "&(syn|rst)" == syn oifname $EXT_NET_IF tcp option maxseg size set rt mtu
PostUp = nft add rule ip nat postrouting oifname $EXT_NET_IF masquerade
...
PostDown = nft delete rule inet filter forward iifname %i accept
PostDown = nft delete rule inet filter forward oifname %i ct state related,established accept
PostDown = nft delete rule ip nat postrouting oifname $EXT_NET_IF masquerade
PostDown = nft delete rule inet filter postrouting ip protocol tcp tcp flags "&(syn|rst)" == syn oifname $EXT_NET_IF tcp option maxseg size set rt mtu
EOF
    if $NET6 && test "$NET6MODE" = "masquerade"; then
        cat << EOF
PostUp = nft add table ip6 nat
PostUp = nft add chain ip6 nat postrouting "{ type nat hook postrouting priority 100 ; }"
PostUp = nft add rule ip6 nat postrouting oifname $EXT_NET_IF masquerade
PostDown = nft delete rule ip6 nat postrouting oifname $EXT_NET_IF masquerade
EOF
    fi
}
